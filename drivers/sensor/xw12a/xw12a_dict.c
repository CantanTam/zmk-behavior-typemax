...
#define PAD_TEN_X_ELEVEN_TEN     (C_VOLUME_UP)
#define PAD_TEN_X_ELEVEN_ELEVEN  (C_VOLUME_DOWN)
#define PAD_NINE_X_ELEVEN_NINE   (C_BRIGHTNESS_INC)
#define PAD_NINE_X_ELEVEN_TEN    (CANCEL)
#define PAD_NINE_X_ELEVEN_ELEVEN (C_BRIGHTNESS_DEC)

//extern bool left_pad_statu;



struct xw12a_pad_keymap {
    uint8_t  padx_key;  // 寄存器地址 (你提到的 0x38, 0x84 是 1 byte)
    uint32_t padx_value;   // 对应的按键值
};

// pad0 pad1 pad2 pad3 使用 left_pad_dict 字典
static const struct xw12a_pad_keymap left_pad_dict[] = {
    { 0x38, PAD_ZERO_ZERO },
    { 0x84, PAD_ONE_ONE   },
    { 0x80, PAD_ZERO_ONE  },
    { 0x3C, PAD_ONE_ZERO  },
...
uint32_t left_dict_addr_padx(uint8_t padx) {
    for (int i = 0; i < LEFT_PAD_DICT_SIZE; i++) { // 永远是正确的次数
        if (left_pad_dict[i].padx_key == padx) {
            return left_pad_dict[i].padx_value;
        }
    }
...
// pad0 pad1 pad2 pad3 使用 left_pad_dict 字典
static const struct xw12a_pad_keymap right_pad_dict[] = {
    { 0x38, PAD_FOUR_FOUR },
    { 0x84, PAD_FIVE_FIVE },
    { 0x80, PAD_FOUR_FIVE },
    { 0x3C, PAD_FIVE_FOUR },
...
uint32_t right_dict_addr_padx(uint8_t padx) {
    for (int i = 0; i < RIGHT_PAD_DICT_SIZE; i++) { // 永远是正确的次数
        if (right_pad_dict[i].padx_key == padx) {
            return right_pad_dict[i].padx_value;
        }
    }
    return 0;
}
...
static const struct xw12a_pad_keymap top_pad_dict[] = {
    // 第一组：84, 88, 90
...
// 紧跟其后，自动计算长度
#define TOP_PAD_DICT_SIZE (sizeof(right_pad_dict) / sizeof(right_pad_dict[0]))

// 然后在查找函数里使用它
uint32_t top_dict_addr_padx(uint8_t padx) {
    for (int i = 0; i < TOP_PAD_DICT_SIZE; i++) { // 永远是正确的次数
        if (top_pad_dict[i].padx_key == padx) {
            return top_pad_dict[i].padx_value;
        }
    }
    return 0;
}